...
# Step 5: separate encoder / decoder layer counts
parser.add_argument('--num-encoder-layers', default=1, type=int,
                    help='Number of Transformer encoder layers (paper uses 1)')
parser.add_argument('--num-decoder-layers', default=3, type=int,
                    help='Number of Transformer decoder layers (paper uses 3)')
# Legacy single-value fallback (overrides both if provided explicitly)
parser.add_argument('--num-layers', default=None, type=int,
                    help='(Legacy) Set both encoder and decoder to the same depth.')
...
# Steps 1-4: SBERT pretrained embedding options
parser.add_argument('--sbert-embeddings', default='', type=str,
                    help='Path to .npy file of SBERT+PCA embeddings '
                         '(generated by sbert_pca_embeddings.py). '
                         'Loads and replaces the encoder embedding weights (steps 1-3).')
parser.add_argument('--freeze-embeddings', action='store_true',
                    help='Freeze the encoder embedding weights after loading (step 4).')
...
parser.add_argument('--lr', default=1e-4, type=float,
                    help='Cross-entropy (main) optimizer learning rate (step 6a: 1e-4)')
parser.add_argument('--mi-lr', default=1e-4, type=float,
                    help='Mutual information optimizer learning rate (step 6b: 1e-4)')
...
if __name__ == '__main__':
    ...
    # Resolve layer counts: --num-layers legacy flag overrides both if provided
    num_enc_layers = args.num_encoder_layers
    num_dec_layers = args.num_decoder_layers
    if args.num_layers is not None:
        num_enc_layers = args.num_layers
        num_dec_layers = args.num_layers

    # Steps 1-3: load SBERT+PCA pretrained embeddings if provided
    import numpy as np
    pretrained_embeddings = None
    if args.sbert_embeddings:
        print(f'Loading SBERT+PCA embeddings from: {args.sbert_embeddings}')
        pretrained_embeddings = np.load(args.sbert_embeddings)
        print(f'  Embedding shape: {pretrained_embeddings.shape}')
    ...
    if args.use_bert_encoder:
        ...
        deepsc = DeepSC(
            ...
            dff=args.dff,
            dropout=0.1,
            # Steps 3-4: inject and optionally freeze SBERT+PCA embeddings
    ...
    # Step 6c: Adam optimizer; only trainable params (frozen embeddings excluded)
    # Step 6a: CE learning rate = 1e-4 (args.lr)
    optimizer = torch.optim.Adam(
        filter(lambda p: p.requires_grad, deepsc.parameters()),
        lr=args.lr, betas=(0.9, 0.98), eps=1e-8, weight_decay=5e-4
    )
    # Step 6b: MI learning rate = 1e-4 (args.mi_lr)
    mi_opt = torch.optim.Adam(mi_net.parameters(), lr=args.mi_lr)
